...
func display_recipe(recipe: CraftingRecipe):
    display_empty() # On utilise votre fonction existante pour vider le slot
    current_recipe = recipe
    
    # On utilise les noms de variables corrects : texture_rect et label
    texture_rect.texture = recipe.output_item.icon
    texture_rect.visible = true
    label.text = "" # Pas de quantité sur l'icône de la recette

    if CraftingManager.can_craft(recipe):
        texture_rect.modulate = Color.WHITE # Couleur normale
    else:
        texture_rect.modulate = Color(0.7, 0.7, 0.7, 0.8) # Grisé
